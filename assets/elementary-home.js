(() => {
  ...
  function currentGrade(summary,prefs){
    ...
    const raw=learner?.grade || learner?.gradeLevel || learner?.stageDetail || "";
    const normalized=window.KhaemenesElementaryContinuity?.normalizeGrade?.(raw) || "";
    return fromContinuity || validGrade(normalized) || prefs.pinnedGrade || "";
  }
  ...
  function renderGrades(activeGrade){
    ...
    for(const grade of GRADES){
      ...
      actions.append(open,tools);article.append(emblem,h,sub,badges,actions);grid.append(article);
    }
  }
  ...
  function mentorPrompt(action){
    const prefs=readPrefs();const summary=continuitySummary();const learner=familyLearner();
    const gradeId=currentGrade(summary,prefs);const grade=GRADES.find(g=>g.id===gradeId)||null;
  ...
  function exportPrefs(){
    ...
    const blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="khaemenes-elementary-preferences.json";document.body.append(a);a.click();a.remove();URL.revokeObjectURL(url);
  }
