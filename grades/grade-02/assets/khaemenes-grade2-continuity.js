/*
 * Khaemenes Grade 02 · Continuity Bridge v1.1.0
 * ------------------------------------------------
 ...
 * Grade 02 owns learner-scoped course progress and mastery records.
...
(function attachGrade2Continuity(global){
  ...
  const LEGACY_CLAIM_KEY="khaemenes_grade2_legacy_migration_claim_v1";
  ...
  function learner(){
    ...
    if(String(summary.learner.grade||"").toLowerCase()!=="grade-02")return null;
  ...
  function normalizeState(value={}){
    ...
    return {student:clean(source.student||"Second Grade Scholar",80),weekly,midterm:Math.max(0,Math.min(100,Number(source.midterm||0))),final:Math.max(0,Math.min(100,Number(source.final||0))),portfolio:Boolean(source.portfolio),learnerId:clean(source.learnerId,120)||null,linkedAt:source.linkedAt||null,updatedAt:source.updatedAt||null,recordVersion:"4.1"};
  }
  ...
  function saveState(value){
    ...
    const records=allRecords();records[l.learnerId]=state;writeJSON(RECORDS_KEY,records);
    return normalizeState(state);
  }

  function clearActive(){
    const l=learner();if(!l)return false;
    const records=allRecords();delete records[l.learnerId];writeJSON(RECORDS_KEY,records);remove(ACTIVE_KEY);return true;
  }
  ...
  global.KhaemenesGrade2Continuity=Object.freeze({version:VERSION,getLearner:learner,getMentor:()=>learner()?.mentor||null,loadState,saveState,clearActive,getSummary:summary,subscribe});
})(window);
