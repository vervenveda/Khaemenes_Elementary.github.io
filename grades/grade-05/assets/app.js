(() => {
  ...
  const CONT = window.KhaemenesGrade5Continuity;
  const $ = id => document.getElementById(id);
  const esc = v => String(v ?? "").replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const blank = () => ({student:"Fifth Grade Scholar", weekly:{}, midterm:0, final:0, portfolio:false});
  ...
  function bind(){
    $("saveProfile").addEventListener("click",()=>{const c=CONT?.status?.();state.student=c?.status==="ready"?c.learner.nickname:($("studentName").value.trim()||"Fifth Grade Scholar");state.midterm=Math.max(0,Math.min(100,Number($("midtermScore").value||0)));state.final=Math.max(0,Math.min(100,Number($("finalScore").value||0)));state.portfolio=$("portfolio").checked;save();renderDashboard();renderWeeks();});
    $("clearRecords").addEventListener("click",()=>{if(!confirm("Clear local fifth-grade records for the active learner on this device?"))return;if(CONT?.clearState)CONT.clearState();else localStorage.removeItem(KEY);state=readState();renderDashboard();renderWeeks();});
  }
